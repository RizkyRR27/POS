...
use App\Models\DetailPenjualModel;
...
class PenjualController extends Controller
{
    ...
    {
        try {
            $sales = PenjualModel::with(['user', 'details'])
                ->select('t_penjualan.*');
    ...
    {
        $sale = PenjualModel::with(['details.barang'])->find($id);
    ...
    public function store_ajax(Request $request)
    {
        ...
        try {
            ...
            $lastId = PenjualModel::max('penjualan_id') ?? 0;
            $penjualan_kode = 'TRX' . str_pad($lastId + 1, 4, '0', STR_PAD_LEFT);

            // Simpan header transaksi
            $sale = PenjualModel::create([
